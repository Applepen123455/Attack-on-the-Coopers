...
try{let saved=JSON.parse(localStorage.ATTACK_COOPERS_SETTINGS||'{}');if(saved&&typeof saved==='object'&&!Array.isArray(saved)){settings.sens=Math.max(.15,Math.min(1,Number(saved.sens)||1));if(saved.binds&&typeof saved.binds==='object'&&!Array.isArray(saved.binds)){let sb={};for(let k of Object.keys(DEFAULT_BINDINGS)){if(typeof saved.binds[k]==='string'&&saved.binds[k].length<40)sb[k]=saved.binds[k]}settings.binds={...DEFAULT_BINDINGS,...sb}}}}catch(e){}
...
function keyLabel(code){return escHtml(code.replace('Key','').replace('Digit','').replace('Arrow','Arrow ').replace('Space','Spacebar'))}
...
function renderSettings(){
  let renderHost=(hostId)=>{
    ...
    host.innerHTML=Object.keys(DEFAULT_BINDINGS).map(a=>`<div class="settingitem"><div class="row"><b>${escHtml(BIND_LABELS[a])}</b></div><button class="bindbtn ${rebinding==a?'listening':''}" data-bind="${escHtml(a)}">${rebinding==a?'Press a key...':keyLabel(settings.binds[a])}</button></div>`).join('');
...
try{let raw=JSON.parse(localStorage.ATTACK_COOPERS_META||'{}');if(raw&&typeof raw==='object'&&!Array.isArray(raw)){meta.wins=safeNum(raw.wins,0);meta.hardWins=safeNum(raw.hardWins,0);meta.epsteinWins=safeNum(raw.epsteinWins,0);meta.unlocks=safeArr(raw.unlocks,[]).filter(u=>typeof u==='string').map(u=>u.slice(0,100))}}catch(e){}
...
function setCharacter(idx){selectedCharacter=Math.max(0,Math.min(CHARACTERS.length-1,idx|0));localStorage.NEON_RELIC_SELECTED_CHAR=selectedCharacter;let c=characterAt(selectedCharacter);PLAYER_IMG.src=c.img;document.querySelectorAll('.charcard').forEach((el,i)=>el.classList.toggle('sel',i===selectedCharacter));let info=$('charInfo');if(info)info.innerHTML=`<div class="row"><b>${escHtml(c.name)}</b><span class="badge">${escHtml(c.role)}</span></div><div class="muted">${escHtml(c.summary)}</div><div class="muted"><b>Ability focus:</b> Shockwave · repair · chain lightning</div><div class="muted"><b>Passive bonus:</b> ${escHtml(c.passive)}</div>`}
function renderCharacterSelect(){let host=$('charSelect');if(!host)return;host.innerHTML=CHARACTERS.map((c,i)=>`<button type="button" class="charcard${i===selectedCharacter?' sel':''}" data-char="${i}"><img src="${escHtml(c.img)}" alt="${escHtml(c.name)}" onerror="this.onerror=null;this.src='images/placeholder-character.png'"><b>${escHtml(c.name)}</b><div class="badge">${escHtml(c.role)}</div><div class="muted" style="margin-top:8px">${escHtml(c.summary)}</div><div class="muted" style="margin-top:6px"><b>Focus:</b> Shockwave · repair · chain lightning</div></button>`).join('');host.querySelectorAll('.charcard').forEach(el=>el.onclick=()=>setCharacter(+el.dataset.char));setCharacter(selectedCharacter)}
...
function setDifficulty(id){
  ...
  let info=$('diffInfo');
  if(info)info.innerHTML=`<div class="row"><b>${escHtml(d.name)}</b><span class="badge">${escHtml(d.badge)}</span></div><div class="muted">${escHtml(d.summary)}</div><div class="muted"><b>Enemy tuning:</b> HP x${d.enemyHp.toFixed(2)}, speed x${d.enemySpeed.toFixed(2)}, damage x${d.enemyDamage.toFixed(2)}</div><div class="muted"><b>Player tuning:</b> hull x${d.playerHull.toFixed(2)}, shield x${d.playerShield.toFixed(2)}, energy x${d.playerEnergy.toFixed(2)}</div>`;
}
function renderDifficultySelect(){
  let host=$('diffSelect'); if(!host) return;
  host.innerHTML=Object.values(DIFFICULTIES).map(d=>`<button type="button" class="charcard diffcard${d.id===selectedDifficulty?' sel':''}" data-diff="${escHtml(d.id)}"><b>${escHtml(d.name)}</b><div class="badge">${escHtml(d.badge)}</div><div class="muted" style="margin-top:8px">${escHtml(d.summary)}</div><div class="muted" style="margin-top:6px"><b>Enemy HP:</b> x${d.enemyHp.toFixed(2)} · <b>Damage:</b> x${d.enemyDamage.toFixed(2)}</div></button>`).join('');
...
function renderShop(){if(!P)return;$('shopScrap').textContent=P.scrap;let items=shopItems();$('shopOptions').innerHTML=items.map((it,i)=>{let can=it[3](),aff=P.scrap>=it[1];let price=Math.ceil(it[1]*(diff().shop||1));return `<button class="shopitem ${(!can||P.scrap<price)?'disabled':''}" data-buy="${i}"><div class="row"><b>${escHtml(it[0])}</b><span class="badge">${price} scrap</span></div><p class="s">${escHtml(it[2])}</p>${!can?'<p class="s">Not available right now.</p>':P.scrap<price?'<p class="s">Need more scrap.</p>':''}</button>`}).join('');[...document.querySelectorAll('[data-buy]')].forEach(btn=>btn.onclick=()=>buyShop(+btn.dataset.buy))}
...
function showForgedGun(gun){craftedGun=gun;let q=gun[9]=='good'?'good':gun[9]=='bad'?'bad':'mid',rar=rarityForGun(gun);$('forgeWeaponBox').innerHTML=`<div class="row"><h2>${escHtml(gun[0])}</h2><span class="${rarityClass(rar)}">${escHtml(rar.toUpperCase())}</span></div><p class="s"><span class="${q}">${escHtml(q.toUpperCase())}</span> · ${escHtml(gun[10])}</p><div class="statgrid"><div class="statpill"><b>${gun[3]}</b><br><span class="s">Damage</span></div><div class="statpill"><b>${gun[1]}s</b><br><span class="s">Fire Rate</span></div><div class="statpill"><b>${gun[5]}</b><br><span class="s">Energy</span></div><div class="statpill"><b>${gun[6]}</b><br><span class="s">Shots</span></div><div class="statpill"><b>${gun[7]}</b><br><span class="s">Pierce</span></div></div><p class="s">Current equipped: ${escHtml(weps[P.w][0])}</p>`;$('forgeResult').style.display='flex'}
...
function safeNum(v,fallback){let n=Number(v);return Number.isFinite(n)?n:fallback}
function safeStr(v,fallback){return typeof v==='string'?v.slice(0,200):fallback}
...
function level(){while(P.xp>=P.nx){P.xp-=P.nx;P.rank++;P.nx=P.nx*1.36+25|0;let u=[...UP].sort(()=>Math.random()-.5).slice(0,4);$('ups').innerHTML=u.map(a=>`<div class="card up"><div class="upgradeBadge">${escHtml(a[0])}</div><h3>${escHtml(a[1])}</h3><p>${escHtml(a[2])}</p></div>`).join('');[...document.querySelectorAll('.up')].forEach((el,i)=>el.onclick=()=>{u[i][3](P);$('level').style.display='none';toast(u[i][1])});$('level').style.display='flex';break}}
...
function ui(){ensureWeaponIndex();let p=(id,v)=>$(id).style.width=cl(v,0,100)+'%';$('hpT').textContent=`${P.h|0}/${P.mh}`;p('hp',100*P.h/P.mh);$('shT').textContent=`${P.s|0}/${P.ms}`;p('sh',100*P.s/P.ms);$('enT').textContent=`${P.e|0}/${P.me}`;p('en',100*P.e/P.me);$('rank').textContent=P.rank;$('xpT').textContent=`XP ${P.xp|0}/${P.nx}`;p('xp',100*P.xp/P.nx);$('rift').textContent=`${P.charName} · ${difficultyAt(selectedDifficulty).badge} · Dungeon Lv ${dungeonLevel()}`;$('seed').textContent=G.seed.toString(16).toUpperCase();$('room').textContent=`${G.rx},${G.ry} ${G.cur.type}`;$('time').textContent=(G.t/60|0)+':'+String(G.t%60|0).padStart(2,'0');$('wep').textContent=weps[P.w][0];$('stats').innerHTML=`<b>${escHtml(P.charName)}</b> · ${escHtml(P.charRole)}<br><span class="muted">${escHtml(P.charPassive)}</span><br>Damage x${P.dm.toFixed(2)} · Armor ${P.armor}<br>Dungeon level ${dungeonLevel()} · Final bosses ${G.bossKills||0}/${G.bossTotal||3}<br>Rooms ${G.rooms.size} · Enemy HP x${levelScale().hp.toFixed(2)}<br>${keyLabel(settings.binds.shock)} Shock ${P.sc<=0?'Ready':P.sc.toFixed(1)} · ${keyLabel(settings.binds.repair)} Repair ${P.rc<=0?'Ready':P.rc.toFixed(1)}<br>${keyLabel(settings.binds.lightning)} Chain Lightning ${P.zc<=0?'Ready':P.zc.toFixed(1)}<br>Heat ${P.heat|0}/100 · Combo x${G.combo} · Mercy ${P.mercy}<br>Kills ${G.kills} · Drones ${G.drones} · Artifacts ${G.art.length}`;$('slots').innerHTML=[['◇',P.scrap],['◆',G.keys],['✦',G.core],['BAT',P.e|0],['DRN',G.drones],['⚙️',P.rank]].map(a=>`<span class="slot">${a[0]}<b>${a[1]}</b></span>`).join('');$('quest').innerHTML=`<b>${escHtml(P.charName)}</b> <span class="badge">${escHtml(P.charRole)}</span> <span class="badge">${escHtml(difficultyAt(selectedDifficulty).badge)}</span><div class="muted" style="margin:6px 0 9px">${escHtml(P.charSummary)}</div><div class="card" style="padding:10px;margin-top:8px"><b>CURRENT OBJECTIVE</b><br>${(G.bossKills||0)>=(G.bossTotal||3)?'Escape: return to spawn and stand on the extraction gate.':`Defeat final bosses: ${G.bossKills||0}/${G.bossTotal||3} destroyed.`}</div><div class="muted" style="margin-top:8px">Enemy clues: laser = sniper, orange ring = bomber, green beam = healer, blue aura = shield.</div><div class="muted" style="margin-top:6px">Unlocks: ${escHtml((meta.unlocks||[]).join(', ')||'none yet')}</div>`}
...
function validateWeapon(w){return Array.isArray(w)&&w.length>=9&&typeof w[0]==='string'&&w[0].length<100&&typeof w[1]==='number'&&typeof w[2]==='number'&&typeof w[3]==='number'}
function load(){
  let d=localStorage.NEON_RELIC_FIXED;if(!d)return toast('No save found');
  try{d=JSON.parse(d)}catch(e){return toast('Corrupted save data')}
  if(!d||typeof d!=='object'||Array.isArray(d))return toast('Invalid save data');
  if(!d.version||d.version<5)return toast('Old save incompatible. Start a new run.');
  if(!d.P||typeof d.P!=='object')return toast('Save data missing player state');
  setDifficulty(safeStr(d.difficulty,selectedDifficulty));
  world(safeNum(d.seed,Date.now()));
  G.t=safeNum(d.t,0);G.keys=safeNum(d.keys,0);G.core=safeNum(d.core,0);G.bossKills=safeNum(d.bossKills,0);G.bossTotal=safeNum(d.bossTotal,BOSS_ROOM_COUNT);G.miniBossTotal=safeNum(d.miniBossTotal,MINI_BOSS_ROOM_COUNT);G.dungeonLevel=safeNum(d.dungeonLevel,1);G.kills=safeNum(d.kills,0);G.drones=safeNum(d.drones,0);G.rift=safeNum(d.rift,1);G.combo=safeNum(d.combo,0);G.comboT=safeNum(d.comboT,0);G.art=safeArr(d.art,[]).filter(a=>typeof a==='string').map(a=>a.slice(0,100));
  G.rooms=new Map(safeArr(d.rooms,[]));
  if(safeArr(d.weps,[]).length&&d.weps.every(validateWeapon))weps.splice(0,weps.length,...d.weps.map(w=>w.slice()));else resetWeapons();
  P={...d.P,seen:new Set(safeArr(d.P&&d.P.seen,['0,0']))};P.w=Math.max(0,Math.min((P.w|0),weps.length-1));
...
function end(w){G.over=1;stopMusic();if(w){meta.wins++;if(selectedDifficulty=='hard')meta.hardWins++;if(selectedDifficulty=='epstein')meta.epsteinWins++;if(selectedDifficulty=='hard')unlockReward('Hard Mode Survivor');if(selectedDifficulty=='epstein')unlockReward('EPSTEIN Conqueror');saveMeta()}let elapsed=Math.max(1,(performance.now()-(runStats.startTime||performance.now()))/1000),title=w?'THE COOPERS ESCAPED':'DUNGEON OVERWHELMED';$('endTitle').textContent=title;$('endSummary').innerHTML=`<b>${escHtml(P.charName)}</b> · ${escHtml(P.charRole)} · <span class="badge">${escHtml(difficultyAt(selectedDifficulty).badge)}</span>`;$('endStats').innerHTML=[['Time',`${(elapsed/60|0)}:${String(elapsed%60|0).padStart(2,'0')}`],['Kills',G.kills],['Rooms visited',runStats.rooms],['Rank reached',P.rank],['Damage dealt',Math.round(runStats.damage)],['Favourite weapon',escHtml(runStats.bestWeapon)],['Difficulty wins',`Easy/Medium total ${meta.wins} · Hard ${meta.hardWins||0} · EPSTEIN ${meta.epsteinWins||0}`],['Unlocks',escHtml((meta.unlocks||[]).join(', ')||'None yet')]].map(([a,b])=>`<div class="card"><div class="t">${escHtml(String(a))}</div><div class="statbig">${escHtml(String(b))}</div></div>`).join('');$('endPanel').style.display='flex'}
