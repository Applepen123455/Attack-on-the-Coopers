...
// ---------------- Multiplayer client ----------------
// This works when the game is opened through server.js, not by double-clicking index.html.
// Put two computers on the same Wi-Fi, run the server on one computer, then both open the same URL.
const MP={
  ws:null,id:null,room:null,seed:null,players:{},connected:false,lastSend:0,
  roomStates:{},bossKills:0,bossTotal:BOSS_ROOM_COUNT
};
function mpToastStatus(){
  let msg=MP.connected?('Multiplayer connected · Room '+MP.room):'Multiplayer offline';
  toast(msg);
}
function mpConnect(){
  ...
  try{
    ...
    MP.room=params.get('room')||'coopers';
    let proto=location.protocol==='https:'?'wss':'ws';
    MP.ws=new WebSocket(proto+'://'+location.host+'/?room='+encodeURIComponent(MP.room));
    MP.ws.onopen=()=>{MP.connected=true;mpToastStatus()};
    MP.ws.onclose=()=>{MP.connected=false;setTimeout(mpConnect,1600)};
    MP.ws.onerror=()=>{MP.connected=false};
    MP.ws.onmessage=e=>{
      let m;try{m=JSON.parse(e.data)}catch(err){return}
      if(m.type==='init'){
        MP.id=m.id;MP.room=m.room;MP.seed=m.seed;MP.bossKills=m.bossKills||0;MP.dungeonLevel=m.dungeonLevel||MP.dungeonLevel||1;MP.bossTotal=m.bossTotal||BOSS_ROOM_COUNT;MP.roomStates=m.roomStates||{};
        if(!P&&!G.run)toast('Multiplayer ready · Room '+MP.room);
      }
      if(m.type==='player'&&m.id!==MP.id){
        MP.players[m.id]={...m,t:performance.now()};
      }
      if(m.type==='leave'){
        delete MP.players[m.id];
      }
      if(m.type==='roomClear'){
        mpApplyRoomClear(m.key,m.payload||{});
      }
      if(m.type==='bossProgress'){
        MP.bossKills=m.bossKills||0;
        if(G&&G.run){
          G.bossKills=Math.max(G.bossKills||0,MP.bossKills);G.dungeonLevel=Math.max(G.dungeonLevel||1,MP.dungeonLevel||1);
          G.bossTotal=m.bossTotal||G.bossTotal||BOSS_ROOM_COUNT;
          if(G.bossKills>=G.bossTotal)G.core=1;
          ui();
        }
      }
      if(m.type==='syncWorld'&&m.seed){
        MP.seed=m.seed;
      }
    };
  }catch(err){}
...
function mpSend(type,data={}){
  if(MP.ws&&MP.ws.readyState===1)MP.ws.send(JSON.stringify({type,...data}));
}
function mpRoomKey(x=G.rx,y=G.ry){return x+','+y}
function mpApplyRoomClear(key,payload={}){
  MP.roomStates[key]={...(MP.roomStates[key]||{}),...payload,clr:1};
  if(G&&G.rooms){
    let r=G.rooms.get(key);
    if(r){
      r.clr=1;
      if(payload.bossDead)r.bossDead=1;
      if(payload.miniReward)r.miniReward=1;
      if(payload.term===0)r.term=0;
      if(G.cur===r)G.en=[];
    }
  }
}
function mpBroadcastClear(r=G.cur){
  if(!r||!MP.connected)return;
  let key=rk(r.x,r.y);
  mpSend('roomClear',{key,payload:{clr:1,bossDead:r.bossDead?1:0,miniReward:r.miniReward?1:0,term:r.term?1:0,type:r.type}});
}
function mpBroadcastBoss(){
  if(!MP.connected)return;
  mpSend('bossProgress',{bossKills:G.bossKills||0,bossTotal:G.bossTotal||BOSS_ROOM_COUNT,dungeonLevel:dungeonLevel()});
}
function mpUpdate(dt){
  if(!P||!G.run||!MP.connected)return;
  MP.lastSend+=dt;
  if(MP.lastSend>=.08){
    MP.lastSend=0;
    mpSend('player',{x:P.x,y:P.y,a:P.a,h:P.h,mh:P.mh,s:P.s,ms:P.ms,rx:G.rx,ry:G.ry,char:P.charName||'Player',role:P.charRole||'',rank:P.rank||1,roomType:G.cur&&G.cur.type});
  }
  let now=performance.now();
  for(let id of Object.keys(MP.players))if(now-MP.players[id].t>3500)delete MP.players[id];
}
function mpDrawPlayers(){
  if(!MP.players)return;
  for(let id of Object.keys(MP.players)){
    let p=MP.players[id];
    if(p.rx!==G.rx||p.ry!==G.ry)continue;
    X.save();
    X.translate(p.x,p.y);
    X.rotate(p.a||0);
    softShadow(0,0,22,.38);
    glow(0,0,72,'#6aff9e',.18);
    X.fillStyle='rgba(106,255,158,.95)';
    X.strokeStyle='rgba(255,255,255,.72)';
    X.lineWidth=2.5;
    X.beginPath();X.arc(0,0,22,0,7);X.fill();X.stroke();
    X.fillStyle='rgba(6,16,20,.9)';
    rr(12,-7,36,14,5);X.fill();
    X.fillStyle='#dff7ff';
    X.font='900 10px sans-serif';
    X.textAlign='center';
    X.fillText('ALLY',0,38);
    X.restore();

    // remote health label
    X.save();
    X.fillStyle='rgba(0,0,0,.72)';
    X.fillRect(p.x-26,p.y-42,52,6);
    X.fillStyle='#6aff9e';
    X.fillRect(p.x-26,p.y-42,52*cl((p.h||1)/(p.mh||1),0,1),6);
    X.fillStyle='#eafff1';
    X.font='900 11px sans-serif';
    X.textAlign='center';
    X.fillText((p.char||'Ally').split(' ')[0],p.x,p.y-50);
    X.restore();
  }
}
function mpSyncGeneratedRooms(){
  if(!G||!G.rooms)return;
  for(let key of Object.keys(MP.roomStates||{}))mpApplyRoomClear(key,MP.roomStates[key]);
  if(MP.bossKills){
    G.bossKills=Math.max(G.bossKills||0,MP.bossKills);
    if(G.bossKills>=(G.bossTotal||BOSS_ROOM_COUNT))G.core=1;
  }
}
mpConnect();


const T=56,RW=18,RH=12,PW=RW*T,PH=RH*T,TARGET_ROOM_COUNT=37,MINI_BOSS_ROOM_COUNT=3,BOSS_ROOM_COUNT=3,STARTER_WEAPON=['Pulse',.11,780,14,.04,2,1,0,'#62e3ff'],weps=[STARTER_WEAPON.slice()];
const FORGE_GUNS=[
...
const DIFFICULTIES={
  easy:{id:'easy',name:'Easy',badge:'Easy',summary:'Still forgiving, but now has real pressure and tougher enemies.',enemyHp:1.02,enemySpeed:.98,enemyDamage:1.12,enemyFire:1.18,enemyBullet:1.02,enemyCount:0,eliteAdd:.00,playerHull:1.02,playerShield:1.06,playerEnergy:1.04,armorAdd:0,shop:.95,xp:1.05,loot:1.08,bossAdd:0,epstein:false},
  medium:{id:'medium',name:'Medium',badge:'Medium',summary:'Harder normal mode with more enemies, stronger bosses, and less free safety.',enemyHp:1.22,enemySpeed:1.08,enemyDamage:1.28,enemyFire:1.08,enemyBullet:1.10,enemyCount:1,eliteAdd:.03,playerHull:.98,playerShield:.94,playerEnergy:.96,armorAdd:0,shop:1.08,xp:.98,loot:.96,bossAdd:0,epstein:false},
  hard:{id:'hard',name:'Hard',badge:'Hard',summary:'Serious dungeon pressure: tougher waves, more elites, and less forgiveness.',enemyHp:1.45,enemySpeed:1.18,enemyDamage:1.52,enemyFire:.98,enemyBullet:1.18,enemyCount:2,eliteAdd:.08,playerHull:.92,playerShield:.84,playerEnergy:.90,armorAdd:-1,shop:1.22,xp:.92,loot:.86,bossAdd:1,epstein:false},
  epstein:{id:'epstein',name:'EPSTEIN Mode',badge:'EPSTEIN',summary:'Hardest mode. Dense enemies, brutal damage, faster pressure, and portrait enemies.',enemyHp:1.7,enemySpeed:1.28,enemyDamage:1.78,enemyFire:.88,enemyBullet:1.25,enemyCount:3,eliteAdd:.14,playerHull:.86,playerShield:.72,playerEnergy:.84,armorAdd:-1,shop:1.38,xp:.84,loot:.74,bossAdd:2,epstein:true}
};
...
const BOSS_VARIANTS=[
  {name:'Warden Core',tag:'WARDEN',col:'#ff426b',hp:1,damage:1,speed:1,shots:0,spread:.16,cool:1},
  {name:'Arc Tyrant',tag:'ARC',col:'#62e3ff',hp:1.18,damage:1.10,speed:1.04,shots:1,spread:.20,cool:.94},
  {name:'Forge Beast',tag:'FORGE',col:'#ff9f1c',hp:1.36,damage:1.20,speed:1.00,shots:2,spread:.23,cool:.90},
  {name:'Void Bishop',tag:'VOID',col:'#b28dff',hp:1.55,damage:1.30,speed:1.08,shots:2,spread:.27,cool:.86},
  {name:'Final Nightmare',tag:'NIGHTMARE',col:'#ff6bcb',hp:1.80,damage:1.45,speed:1.14,shots:3,spread:.30,cool:.82}
];
function bossVariant(level=dungeonLevel()){
  return BOSS_VARIANTS[Math.min(BOSS_VARIANTS.length-1,Math.floor((Math.max(1,level)-1)/5))];
}
function bossVariantName(level=dungeonLevel()){
  let v=bossVariant(level);
  return v.name+' Lv '+level;
}
...
function spawnRoom(r){
  ...
  if(r.boss){
    ...
    enemy('boss',PW/2,175);let be=G.en[G.en.length-1],bv=bossVariant(ls.L);be.variant=bv;be.variantName=bv.name;be.variantTag=bv.tag;be.col=bv.col;be.h*=bv.hp*(1+(ls.L-1)*.18);be.mh=be.h;be.sp*=bv.speed*(1+(ls.L-1)*.035);be.dmg*=bv.damage*(1+(ls.L-1)*.09);
    toast(bossVariantName(ls.L)+' · FINAL BOSS '+(r.bossId||'?')+' / '+(G.bossTotal||3));
    return
  }
  if(r.miniboss){
...
function upEnemies(dt){for(let e of G.en){e.fl=Math.max(0,e.fl-dt);e.cd-=dt;let target=nearestDecoy(e.x,e.y)||P,a=Math.atan2(target.y-e.y,target.x-e.x),d=di(e.x,e.y,target.x,target.y);if(['drone','split'].includes(e.type)){e.vx+=Math.cos(a)*e.sp*dt*2.6;e.vy+=Math.sin(a)*e.sp*dt*2.6}if(e.type=='swarm'){e.vx+=Math.cos(a+Math.sin(G.t*8)*.6)*e.sp*dt*3.1;e.vy+=Math.sin(a+Math.sin(G.t*8)*.6)*e.sp*dt*3.1}if(e.type=='dash'){e.vx+=Math.cos(a)*e.sp*dt;if(e.cd<=0){e.cd=2.5*diff().enemyFire;e.vx+=Math.cos(a)*640*diff().enemySpeed;e.vy+=Math.sin(a)*640*diff().enemySpeed}}if(['turret','tank','sniper','shield'].includes(e.type)){if(e.type=='tank'||e.type=='shield'){e.vx+=Math.cos(a)*e.sp*dt;e.vy+=Math.sin(a)*e.sp*dt}if(e.type=='sniper'&&d<380){e.vx-=Math.cos(a)*e.sp*dt*3;e.vy-=Math.sin(a)*e.sp*dt*3}if(e.type=='shield')for(let o of G.en)if(o!==e&&di(e.x,e.y,o.x,o.y)<140)o.h=Math.min(o.mh,o.h+3*dt);if(e.cd<=0&&d<780){e.cd=(e.type=='sniper'?2.2:e.type=='tank'?1.2:e.type=='shield'?2.4:1.7)*diff().enemyFire;shootE(e,e.type=='tank'?5:e.type=='shield'?6:e.type=='sniper'?1:3)}}if(e.type=='bomber'){e.vx+=Math.cos(a)*e.sp*dt*2.5;e.vy+=Math.sin(a)*e.sp*dt*2.5;if(d<55){explodeEnemy(e)}}if(e.type=='healer'){e.vx-=Math.cos(a)*e.sp*dt*.7;e.vy-=Math.sin(a)*e.sp*dt*.7;if(e.cd<=0){e.cd=1.5*diff().enemyFire;let low=G.en.filter(o=>o!==e&&o.h<o.mh&&di(e.x,e.y,o.x,o.y)<260)[0];if(low){low.h=Math.min(low.mh,low.h+35*diff().enemyHp);ring(low.x,low.y,'#9cffb1',15,65,.35)}else if(d<620)shootE(e,2)}}if(e.type=='boss'){let phase=e.h<e.mh*.33?3:e.h<e.mh*.66?2:1,bv=e.variant||bossVariant();e.phase=phase;e.vx+=Math.cos(a)*e.sp*dt*(.70+phase*.03);e.vy+=Math.sin(a)*e.sp*dt*(.10+phase*.02);if(e.cd<=0){e.cd=(phase==3?1.35:phase==2?1.55:1.85)*diff().enemyFire*(bv.cool||1);let shots=(phase==3?7:phase==2?6:5)+(bv.shots||0),base=Math.atan2(P.y-e.y,P.x-e.x),spread=bv.spread||.16;for(let i=0;i<shots;i++)eb(e.x,e.y,base+(i-(shots-1)/2)*spread,190*diff().enemyBullet*levelScale().bullets,e.col,13*diff().enemyDamage*(bv.damage||1));if(phase>=2&&G.eb.length<70){for(let i=0;i<Math.max(0,(bv.shots||0));i++){let aa=G.t+i*6.28/Math.max(1,bv.shots);eb(e.x,e.y,aa,150*diff().enemyBullet,e.col,10*e.dmg)}}if(G.rings.length<18)ring(e.x,e.y,e.col,30,115+phase*12,.32);G.shake=4+phase}}if(e.slow>0){e.slow-=dt;e.vx*=.78;e.vy*=.78}e.vx*=.94;e.vy*=.94;move(e,dt);if(d<P.r+e.r&&P.iv<=0&&!target.fake)hurt((e.type=='boss'?25:14)*e.dmg)}G.en=G.en.filter(e=>!e.dead)}
...
function kill(e){
  ...
  if(e.type=='boss'){
    ...
    if(G.bossKills>=(G.bossTotal||BOSS_ROOM_COUNT)){
      G.core=1;
      drop(e.x,e.y,'core',1);
      toast('ALL 3 FINAL BOSSES DEAD. RETURN TO SPAWN.');mpBroadcastBoss();mpBroadcastClear(G.cur);
    }else{
      toast('Final boss defeated: '+G.bossKills+'/'+(G.bossTotal||BOSS_ROOM_COUNT));mpBroadcastBoss();mpBroadcastClear(G.cur);
...
function state(){
  if(!G.cur.clr&&G.en.length==0){let oldDungeonLevel=dungeonLevel();
    G.cur.clr=1;
    ...
    if(G.cur.miniboss&&!G.cur.miniReward){
      ...
      toast('Mini-boss room cleared');mpBroadcastClear(G.cur)
    }else {toast('Room clear');mpBroadcastClear(G.cur)}
  }
  let newDungeonLevel=dungeonLevel();if(newDungeonLevel>oldDungeonLevel){G.dungeonLevel=newDungeonLevel;toast('DUNGEON LEVEL '+newDungeonLevel+' - enemies grew stronger')}if(G.rx==0&&G.ry==0&&(G.bossKills||0)>=(G.bossTotal||BOSS_ROOM_COUNT)&&di(P.x,P.y,PW/2,PH/2)<95)end(1)
}
...
function simpleBossDraw(e){
  ...
  let bv=e.variant||bossVariant();glow(e.x,e.y,e.r*2.8,bv.col||'#ff4d6d',.15);
  X.save();
  X.translate(e.x,e.y);
  ...
  grad.addColorStop(.55,bv.col||'#8c1f35');
  ...
  X.font='900 12px sans-serif';
  X.textAlign='center';
  X.fillText((bv.tag||'BOSS')+(e.phase?' P'+e.phase:''),0,e.r+18);
  X.restore();
}
...
function ui(){ensureWeaponIndex();let p=(id,v)=>$(id).style.width=cl(v,0,100)+'%';$('hpT').textContent=`${P.h|0}/${P.mh}`;p('hp',100*P.h/P.mh);$('shT').textContent=`${P.s|0}/${P.ms}`;p('sh',100*P.s/P.ms);$('enT').textContent=`${P.e|0}/${P.me}`;p('en',100*P.e/P.me);$('rank').textContent=P.rank;$('xpT').textContent=`XP ${P.xp|0}/${P.nx}`;p('xp',100*P.xp/P.nx);$('rift').textContent=`${P.charName} · ${difficultyAt(selectedDifficulty).badge} · Dungeon Lv ${dungeonLevel()}`;$('seed').textContent=G.seed.toString(16).toUpperCase();$('room').textContent=`${G.rx},${G.ry} ${G.cur.type}`;$('time').textContent=(G.t/60|0)+':'+String(G.t%60|0).padStart(2,'0');$('wep').textContent=weps[P.w][0];$('stats').innerHTML=`<b>${P.charName}</b> · ${P.charRole}<br><span class="muted">${P.charPassive}</span><br>Damage x${P.dm.toFixed(2)} · Armor ${P.armor}<br>Dungeon level ${dungeonLevel()} · Boss: ${bossVariant().name}<br>Final bosses ${G.bossKills||0}/${G.bossTotal||3}<br>Rooms ${G.rooms.size} · Enemy HP x${levelScale().hp.toFixed(2)}<br>${keyLabel(settings.binds.shock)} Shock ${P.sc<=0?'Ready':P.sc.toFixed(1)} · ${keyLabel(settings.binds.repair)} Repair ${P.rc<=0?'Ready':P.rc.toFixed(1)}<br>${keyLabel(settings.binds.lightning)} Chain Lightning ${P.zc<=0?'Ready':P.zc.toFixed(1)}<br>Heat ${P.heat|0}/100 · Combo x${G.combo} · Mercy ${P.mercy}<br>Kills ${G.kills} · Drones ${G.drones} · Artifacts ${G.art.length}`;$('slots').innerHTML=[['◇',P.scrap],['◆',G.keys],['✦',G.core],['🔋',P.e|0],['🤖',G.drones],['⚙️',P.rank]].map(a=>`<span class="slot">${a[0]}<b>${a[1]}</b></span>`).join('');$('quest').innerHTML=`<b>${P.charName}</b> <span class="badge">${P.charRole}</span> <span class="badge">${difficultyAt(selectedDifficulty).badge}</span><div class="muted" style="margin:6px 0 9px">${P.charSummary}</div><div class="card" style="padding:10px;margin-top:8px"><b>CURRENT OBJECTIVE</b><br>${(G.bossKills||0)>=(G.bossTotal||3)?'Escape: return to spawn and stand on the extraction gate.':`Defeat final bosses: ${G.bossKills||0}/${G.bossTotal||3} destroyed.`}</div><div class="muted" style="margin-top:8px">Enemy clues: laser = sniper, orange ring = bomber, green beam = healer, blue aura = shield.</div><div class="muted" style="margin-top:6px">Multiplayer: ${MP.connected?'online room '+MP.room:'offline'} · Dungeon Lv ${dungeonLevel()}</div><div class="muted" style="margin-top:6px">Unlocks: ${(meta.unlocks||[]).join(', ')||'none yet'}</div>`}
...
function load(){
  ...
  G.run=1;G.pause=0;G.over=0;ms.d=0;K.clear();mpSyncGeneratedRooms();renderSettings();ui();toast('Loaded')
}
