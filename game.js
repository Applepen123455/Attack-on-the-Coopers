...
function spawnRoom(r){
  ...
  if(r.miniboss){
    ...
    let makeMini=(type,x,y)=>{
      ...
      e.h*=1.9;e.mh=e.h;e.r*=1.2;e.sp*=1.08;e.xp=Math.round(e.xp*2.8);
    };
    ...
    let adds=Math.max(4,3+(d.enemyCount||0));
    for(let i=0;i<adds;i++){
      let x,y;do{x=R(PW-180,90);y=R(PH-180,90)}while(di(x,y,P.x,P.y)<260);
      enemy(pick(['swarm','dash','drone','bomber']),x,y)
    }
    toast('MINI-BOSS ROOM '+(r.miniId||''));
    return
  }
  let n=Math.max(4,Math.floor(5+Math.abs(r.x)+Math.abs(r.y)+G.rift*1.7+(d.enemyCount||0)));
  for(let i=0;i<n;i++){
    let x,y;do{x=R(PW-180,90);y=R(PH-180,90)}while(di(x,y,P.x,P.y)<260);
...
function ui(){ensureWeaponIndex();let p=(id,v)=>$(id).style.width=cl(v,0,100)+'%';$('hpT').textContent=`${P.h|0}/${P.mh}`;p('hp',100*P.h/P.mh);$('shT').textContent=`${P.s|0}/${P.ms}`;p('sh',100*P.s/P.ms);$('enT').textContent=`${P.e|0}/${P.me}`;p('en',100*P.e/P.me);$('rank').textContent=P.rank;$('xpT').textContent=`XP ${P.xp|0}/${P.nx}`;p('xp',100*P.xp/P.nx);$('rift').textContent=`${P.charName} · ${difficultyAt(selectedDifficulty).badge} · Rift ${G.rift}`;$('seed').textContent=G.seed.toString(16).toUpperCase();$('room').textContent=`${G.rx},${G.ry} ${G.cur.type}`;$('time').textContent=(G.t/60|0)+':'+String(G.t%60|0).padStart(2,'0');$('wep').textContent=weps[P.w][0];$('stats').innerHTML=`<b>${P.charName}</b> · ${P.charRole}<br><span class="muted">${P.charPassive}</span><br>Damage x${P.dm.toFixed(2)} · Armor ${P.armor}<br>Final bosses ${G.bossKills||0}/${G.bossTotal||3} · Rooms ${G.rooms.size}<br>${keyLabel(settings.binds.shock)} Shock ${P.sc<=0?'Ready':P.sc.toFixed(1)} · ${keyLabel(settings.binds.repair)} Repair ${P.rc<=0?'Ready':P.rc.toFixed(1)}<br>${keyLabel(settings.binds.lightning)} Chain Lightning ${P.zc<=0?'Ready':P.zc.toFixed(1)}<br>Heat ${P.heat|0}/100 · Combo x${G.combo} · Mercy ${P.mercy}<br>Kills ${G.kills} · Drones ${G.drones} · Artifacts ${G.art.length}`;$('slots').innerHTML=[['◇',P.scrap],['◆',G.keys],['✦',G.core],['🔋',P.e|0],['🤖',G.drones],['⚙️',P.rank]].map(a=>`<span class="slot">${a[0]}<b>${a[1]}</b></span>`).join('');$('quest').innerHTML=`<b>${P.charName}</b> <span class="badge">${P.charRole}</span> <span class="badge">${difficultyAt(selectedDifficulty).badge}</span><div class="muted" style="margin:6px 0 9px">${P.charSummary}</div><div class="card" style="padding:10px;margin-top:8px"><b>CURRENT OBJECTIVE</b><br>${(G.bossKills||0)>=(G.bossTotal||3)?'Escape: return to spawn and stand on the extraction gate.':`Defeat final bosses: ${G.bossKills||0}/${G.bossTotal||3} destroyed.`}</div><div class="muted" style="margin-top:8px">Enemy clues: laser = sniper, orange ring = bomber, green beam = healer, blue aura = shield.</div><div class="muted" style="margin-top:6px">Unlocks: ${(meta.unlocks||[]).join(', ')||'none yet'}</div>`}
...
function save(){
  ...
  localStorage.NEON_RELIC_FIXED=JSON.stringify({version:5,seed:G.seed,t:G.t,rx:G.rx,ry:G.ry,difficulty:selectedDifficulty,P:{...P,seen:[...P.seen]},weps:weps.map(w=>w.slice()),keys:G.keys,core:G.core,bossKills:G.bossKills||0,bossTotal:G.bossTotal||BOSS_ROOM_COUNT,miniBossTotal:G.miniBossTotal||MINI_BOSS_ROOM_COUNT,kills:G.kills,drones:G.drones,rooms:[...G.rooms],en:G.en,lt:G.lt,mines:G.mines,sentries:G.sentries,decoys:G.decoys,barriers:G.barriers,art:G.art,rift:G.rift,combo:G.combo,comboT:G.comboT});
  toast('Saved');
  return true
}
function load(){
  ...
  world(d.seed);
  G.t=d.t||0;G.keys=d.keys||0;G.core=d.core||0;G.bossKills=d.bossKills||0;G.bossTotal=d.bossTotal||BOSS_ROOM_COUNT;G.miniBossTotal=d.miniBossTotal||MINI_BOSS_ROOM_COUNT;G.kills=d.kills||0;G.drones=d.drones||0;G.rift=d.rift||1;G.combo=d.combo||0;G.comboT=d.comboT||0;G.art=d.art||[];
