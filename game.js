...
{let saved=localGet('ATTACK_COOPERS_SETTINGS',{});settings.sens=Number(saved.sens)||1;settings.binds={...DEFAULT_BINDINGS,...(saved.binds||{})}}

function openPause(){if(!P||!G.run)return;G.pause=1;renderSettings();showPanel('pausePanel')}
function closePause(){G.pause=0;hidePanel('pausePanel')}

function saveSettings(){localSet('ATTACK_COOPERS_SETTINGS',settings)}
...
function renderSettings(){
  ...
  let onSensChange=v=>{settings.sens=v;syncSlider('sens','sensLabel',v,()=>{});syncSlider('pauseSens','pauseSensLabel',v,()=>{});saveSettings()};
  syncSlider('sens','sensLabel',settings.sens,onSensChange);
  syncSlider('pauseSens','pauseSensLabel',settings.sens,onSensChange);
...
// --- Shared Utilities ---
function showPanel(id){let el=$(id);if(el)el.style.display='flex'}
function hidePanel(id){let el=$(id);if(el)el.style.display='none'}
function isPanelOpen(id){let el=$(id);return el&&el.style.display==='flex'}
function drawHealthBar(x,y,w,h,current,max){X.fillStyle='rgba(0,0,0,.72)';X.fillRect(x,y,w,h);let hb=X.createLinearGradient(x,0,x+w,0);hb.addColorStop(0,'#ff426b');hb.addColorStop(1,'#ffb0bf');X.fillStyle=hb;X.fillRect(x,y,w*cl(current/max,0,1),h)}
function safeSpawnPos(safeX,safeY,safeDist){let x,y;do{x=R(PW-180,90);y=R(PH-180,90)}while(di(x,y,safeX,safeY)<safeDist);return{x,y}}
function localGet(key,fallback){try{let v=localStorage[key];return v?JSON.parse(v):fallback}catch(e){return fallback}}
function localSet(key,value){localStorage[key]=JSON.stringify(value)}
function syncSlider(sliderId,labelId,value,onChange){let s=$(sliderId),sl=$(labelId);if(!s||!sl)return;s.value=value;sl.textContent=Number(value).toFixed(2);s.oninput=e=>{let v=parseFloat(e.target.value);onChange(v);sl.textContent=v.toFixed(2)}}
// --- End Shared Utilities ---

function drawMicroDust(){X.save();X.globalCompositeOperation='screen';for(let i=0;i<42;i++){let x=(i*149+G.t*18)%W,y=(i*263+G.t*9)%H;X.fillStyle='rgba(180,230,255,.035)';X.beginPath();X.arc(x,y,1+(i%3)*.35,0,7);X.fill()}X.restore()}
...
function spawnRoom(r){
  ...
  if(r.miniboss){
    ...
    for(let i=0;i<adds;i++){
      let pos=safeSpawnPos(P.x,P.y,260);
      enemy(pick(['swarm','dash','drone','bomber']),pos.x,pos.y)
    }
  ...
  for(let i=0;i<n;i++){
    let pos=safeSpawnPos(P.x,P.y,260);
    enemy(pick(['drone','drone','dash','turret','split','tank','sniper','bomber','healer','shield','swarm','swarm','swarm']),pos.x,pos.y)
  }
}
...
function openShop(){showPanel('shopPanel');renderShop();toast('Market opened')}
function closeShop(){hidePanel('shopPanel')}
...
function openForgePuzzle(){if(G.cur.used)return toast('Forge already used');puzzle=makeForgePuzzle();puzzle.left=puzzle.max;puzzle.start=performance.now();$('puzzleTitle').textContent='Forge Calibration Puzzle';$('puzzleIntro').textContent='Solve a clean forge terminal puzzle to craft a random gun. Some forged guns are powerful. Some are awful. Exiting does not consume the forge.';showPanel('decodePanel');$('puzzlePrompt').textContent=puzzle.prompt;$('puzzleAnswer').value='';$('puzzleMsg').textContent='';renderPuzzleChoices();$('puzzleTimer').textContent=Math.ceil(puzzle.max)+'s';setTimeout(()=>$('puzzleAnswer').focus(),40);toast('Forge puzzle opened')}
...
function showForgedGun(gun){craftedGun=gun;let q=gun[9]=='good'?'good':gun[9]=='bad'?'bad':'mid',rar=rarityForGun(gun);$('forgeWeaponBox').innerHTML=`<div class="row"><h2>${gun[0]}</h2><span class="${rarityClass(rar)}">${rar.toUpperCase()}</span></div><p class="s"><span class="${q}">${q.toUpperCase()}</span> · ${gun[10]}</p><div class="statgrid"><div class="statpill"><b>${gun[3]}</b><br><span class="s">Damage</span></div><div class="statpill"><b>${gun[1]}s</b><br><span class="s">Fire Rate</span></div><div class="statpill"><b>${gun[5]}</b><br><span class="s">Energy</span></div><div class="statpill"><b>${gun[6]}</b><br><span class="s">Shots</span></div><div class="statpill"><b>${gun[7]}</b><br><span class="s">Pierce</span></div></div><p class="s">Current equipped: ${weps[P.w][0]}</p>`;showPanel('forgeResult')}
function equipForgedGun(){if(!craftedGun)return;weps[ensureWeaponIndex()]=craftedGun.slice(0,9);toast('Equipped '+craftedGun[0]);craftedGun=null;hidePanel('forgeResult');ui()}
function skipForgedGun(){craftedGun=null;hidePanel('forgeResult');toast('Kept current gun')}
...
function openPuzzle(){if(!G.cur.term)return toast('Archive already decoded');resetPuzzleText();puzzle=makePuzzle();puzzle.max=35;puzzle.left=35;puzzle.start=performance.now();showPanel('decodePanel');$('puzzlePrompt').textContent=puzzle.prompt;$('puzzleAnswer').value='';$('puzzleMsg').textContent='';renderPuzzleChoices();$('puzzleTimer').textContent='35s';setTimeout(()=>$('puzzleAnswer').focus(),40);toast('Archive puzzle opened')}
function closePuzzle(){hidePanel('decodePanel');if($('puzzleChoices'))$('puzzleChoices').innerHTML='';resetPuzzleText();puzzle=null;toast('Puzzle exited')}
function updatePuzzleTimer(){if(!puzzle||!isPanelOpen('decodePanel'))return;puzzle.left=Math.max(0,puzzle.max-(performance.now()-puzzle.start)/1000);$('puzzleTimer').textContent=Math.ceil(puzzle.left)+'s';if(puzzle.left<=0){$('puzzleMsg').textContent='Time bonus is gone, but you can still solve it for base XP.'}}
function submitPuzzle(){if(!puzzle)return;let norm=s=>String(s).trim().replace(/\s+/g,' ').toLowerCase();let ans=norm($('puzzleAnswer').value);if(ans!==norm(puzzle.answer)){ sfx('fail');$('puzzleMsg').textContent='Incorrect. Hint: '+(puzzle.hint||'Look closely at the pattern.');return }let ratio=cl(puzzle.left/puzzle.max,0,1);hidePanel('decodePanel');if(puzzle.forge){G.cur.used=1;let gun=rollForgedGun(ratio);ring(PW/2,PH/2,'#ff9f1c',30,180,.75);burst(PW/2,PH/2,'#ff9f1c',100,5,460);puzzle=null;resetPuzzleText();showForgedGun(gun);sfx('forge');toast('Weapon forged');return}let gain=Math.floor((25+Math.floor(85*ratio))*(diff().xp||1));G.cur.term=0;P.xp+=gain;ring(PW/2,PH/2,'#62e3ff',30,170,.7);burst(PW/2,PH/2,'#62e3ff',90,5,420);puzzle=null;resetPuzzleText();sfx('puzzle');toast('Archive decoded: +'+gain+' XP');level();ui()}
...
function level(){while(P.xp>=P.nx){P.xp-=P.nx;P.rank++;P.nx=P.nx*1.36+25|0;let u=[...UP].sort(()=>Math.random()-.5).slice(0,4);$('ups').innerHTML=u.map(a=>`<div class="card up"><div class="upgradeBadge">${a[0]}</div><h3>${a[1]}</h3><p>${a[2]}</p></div>`).join('');[...document.querySelectorAll('.up')].forEach((el,i)=>el.onclick=()=>{u[i][3](P);hidePanel('level');toast(u[i][1])});showPanel('level');break}}
...

{
  ...
  X.restore();
  drawHealthBar(e.x-e.r,e.y-e.r-16,e.r*2,6,e.h,e.mh);
  return;
}
...
function load(){
  ...
  hidePanel('menu');hidePanel('level');$('codex').style.display='none';$('exitRun').style.display='block';$('pauseRun').style.display='block';
...
function exitRun(){if(!P||!G.run)return;save();G.run=0;G.pause=0;hidePanel('level');$('codex').style.display='none';$('exitRun').style.display='none';$('pauseRun').style.display='none';showPanel('menu');renderCharacterSelect();renderDifficultySelect();$('start').onclick=start;$('load').onclick=load;if($('audioToggle'))$('audioToggle').onclick=toggleAudio;toast('Progress saved')}
function end(w){G.over=1;stopMusic();if(w){meta.wins++;if(selectedDifficulty=='hard')meta.hardWins++;if(selectedDifficulty=='epstein')meta.epsteinWins++;if(selectedDifficulty=='hard')unlockReward('Hard Mode Survivor');if(selectedDifficulty=='epstein')unlockReward('EPSTEIN Conqueror');saveMeta()}let elapsed=Math.max(1,(performance.now()-(runStats.startTime||performance.now()))/1000),title=w?'THE COOPERS ESCAPED':'DUNGEON OVERWHELMED';$('endTitle').textContent=title;$('endSummary').innerHTML=`<b>${P.charName}</b> · ${P.charRole} · <span class="badge">${difficultyAt(selectedDifficulty).badge}</span>`;$('endStats').innerHTML=[['Time',`${(elapsed/60|0)}:${String(elapsed%60|0).padStart(2,'0')}`],['Kills',G.kills],['Rooms visited',runStats.rooms],['Rank reached',P.rank],['Damage dealt',Math.round(runStats.damage)],['Favourite weapon',runStats.bestWeapon],['Difficulty wins',`Easy/Medium total ${meta.wins} · Hard ${meta.hardWins||0} · EPSTEIN ${meta.epsteinWins||0}`],['Unlocks',(meta.unlocks||[]).join(', ')||'None yet']].map(([a,b])=>`<div class="card"><div class="t">${a}</div><div class="statbig">${b}</div></div>`).join('');showPanel('endPanel')}
...
function start(){setCharacter(selectedCharacter);setDifficulty(selectedDifficulty);world();ensureWeaponIndex();startMusic();hidePanel('menu');$('exitRun').style.display='block';$('pauseRun').style.display='block';ui();toast('Objective shown on right · '+P.charName+' deployed')}
