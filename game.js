...
const T=56,RW=18,RH=12,PW=RW*T,PH=RH*T,STARTER_WEAPON=['Pulse',.11,780,14,.04,2,1,0,'#62e3ff'],weps=[STARTER_WEAPON.slice()];
const FORGE_GUNS=[
...
function renderCharacterSelect(){let host=$('charSelect');if(!host)return;host.innerHTML=CHARACTERS.map((c,i)=>`<button type="button" class="charcard${i===selectedCharacter?' sel':''}" data-char="${i}"><img src="${c.img}" alt="${c.name}"><b>${c.name}</b><div class="badge">${c.role}</div><div class="muted" style="margin-top:8px">${c.summary}</div><div class="muted" style="margin-top:6px"><b>Focus:</b> Shockwave · repair · chain lightning</div></button>`).join('');host.querySelectorAll('.charcard').forEach(el=>el.onclick=()=>setCharacter(+el.dataset.char));setCharacter(selectedCharacter)}
...
const DIFFICULTIES={
  easy:{id:'easy',name:'Easy',badge:'Easy',summary:'Lower pressure, fewer threats, and more forgiving player stats.',enemyHp:.84,enemySpeed:.9,enemyDamage:.95,enemyFire:1.32,enemyBullet:.92,enemyCount:-1,eliteAdd:-.02,playerHull:1.08,playerShield:1.22,playerEnergy:1.1,armorAdd:1,shop:0.85,xp:1.15,loot:1.2,bossAdd:0,epstein:false},
  medium:{id:'medium',name:'Medium',badge:'Medium',summary:'The normal balance of the game.',enemyHp:1,enemySpeed:1,enemyDamage:1.10,enemyFire:1.25,enemyBullet:1,enemyCount:0,eliteAdd:0,playerHull:1,playerShield:1,playerEnergy:1,armorAdd:0,shop:1,xp:1,loot:1,bossAdd:0,epstein:false},
  hard:{id:'hard',name:'Hard',badge:'Hard',summary:'Tougher enemies, slower enemy shots, harder hits, and more room pressure.',enemyHp:1.2,enemySpeed:1.1,enemyDamage:1.30,enemyFire:1.14,enemyBullet:1.08,enemyCount:1,eliteAdd:.04,playerHull:.97,playerShield:.92,playerEnergy:.96,armorAdd:0,shop:1.16,xp:.94,loot:.9,bossAdd:1,epstein:false},
  epstein:{id:'epstein',name:'EPSTEIN Mode',badge:'EPSTEIN',summary:'Hardest mode. Enemies are stronger, denser, hit harder, shoot slower, and all use the attached portrait as their face.',enemyHp:1.38,enemySpeed:1.18,enemyDamage:1.48,enemyFire:1.04,enemyBullet:1.15,enemyCount:2,eliteAdd:.08,playerHull:.94,playerShield:.82,playerEnergy:.92,armorAdd:-1,shop:1.28,xp:.88,loot:.8,bossAdd:2,epstein:true}
};
...
const UP=[['❤️','Reinforced Hull','+35 max hull and repair.',p=>{p.mh+=35;p.h=cl(p.h+60,0,p.mh)}],['🛡️','Aegis Loop','+25 shield and regen.',p=>{p.ms+=25;p.sr+=2;p.s=p.ms}],['🔥','Overclock','+20% damage.',p=>p.dm*=1.2],['🌀','Phase Legs','Faster movement and dash.',p=>{p.sp*=1.12;p.dcMax*=.85}],['🤖','Combat Drone','Adds a drone.',p=>G.drones++],['🎯','Crit Oracle','Better critical hits.',p=>{p.cr+=.08;p.cm+=.35}],['🧲','Magnet Array','Bigger pickup range.',p=>p.mag+=80],['⚡','Reactor Blood','Kills refund energy.',p=>p.ref+=3],['✹','Splinter Rounds','Enemies burst into bullets.',p=>p.spl++],['⛈️','Storm Heart','Better shockwave.',p=>{p.qc*=.75;p.qr+=55}],['🛠️','Field Medic','Better repair pulse.',p=>p.repairPow+=.4],['⚡','Storm Conductor','Better chain lightning.',p=>p.chainPow+=.5],['🔫','Gun Mastery','Shots deal +12% damage and cost slightly less energy.',p=>{p.dm*=1.12;p.er+=1.2}],['🧱','Shield Plating','Gain +18 shield, +1 armor, and faster shield recharge.',p=>{p.ms+=18;p.sr+=1.6;p.armor+=1;p.s=p.ms}],['🌿','Living Alloy','Passive hull regen.',p=>p.passive+=1.1],['🦾','Mercy Protocol','One extra revive.',p=>p.mercy++],['🔋','Energy Reservoir','Gain +22 max energy and faster energy recharge.',p=>{p.me+=22;p.er+=2.4;p.e=p.me}],['🍖','Brisket Rounds','Bullets hit harder and critical chance increases.',p=>{p.dm*=1.08;p.cr+=.06}],['👟','Dungeon Runner','Move faster and gain a shorter dash cooldown.',p=>{p.sp*=1.08;p.dcMax*=.9}],['🧠','Tactical Study','Level up faster and improve repair power.',p=>{p.nx=Math.max(40,p.nx*.88|0);p.repairPow+=.18}],['💰','Scrap Synth','More scrap drops.',p=>p.scrapMul*=1.35]];
function player(){return applyDifficultyToPlayer(applyCharacter({x:PW/2,y:PH/2,r:22,vx:0,vy:0,a:0,h:135,mh:135,s:35,ms:35,e:115,me:115,sr:7,er:13,sp:315,w:0,rank:1,xp:0,nx:60,scrap:25,ft:0,iv:0,dc:0,dcMax:.75,sc:0,qc:4.2,qr:165,tc:0,rc:0,mc:0,zc:0,xc:0,bc:0,gc:0,over:0,heat:0,dm:1.08,cr:.08,cm:1.75,mag:125,ref:1,spl:0,armor:2,mercy:1,passive:0,timePow:1,repairPow:1,minePow:1,chainPow:1,decoyPow:1,sentryPow:1,barrierPow:1,scrapMul:1,seen:new Set(['0,0']),shopLock:0,recoil:0,breath:0},selectedCharacter))}
const rk=(x,y)=>x+','+y;function room(x,y,type){let special=G.rng&&G.rng.n()<.13&&type=='combat',rt=special?G.rng.p(['shrine','rescue','forge']):type,r={x,y,type:rt,clr:type=='start'||type=='shop'||type=='lore'||special,doors:{},obs:[],haz:[],loot:[],spawn:0,key:0,boss:0,term:type=='lore',used:0};for(let i=0;i<(type=='start'?3:G.rng.i(5,12));i++){let w=G.rng.i(1,3)*T,h=G.rng.i(1,2)*T,ox=G.rng.i(2,RW-4)*T,oy=G.rng.i(2,RH-4)*T;if(di(ox,oy,PW/2,PH/2)>170)r.obs.push({x:ox,y:oy,w,h})}if(!r.clr&&G.rng.n()<.35)for(let i=0;i<G.rng.i(2,4);i++)r.haz.push({x:G.rng.i(2,RW-3)*T,y:G.rng.i(2,RH-3)*T,r:G.rng.i(28,44),p:G.rng.r(0,6.28)});return r}
function world(seed=Date.now()){resetWeapons();runStats={damage:0,shots:0,rooms:0,startTime:performance.now(),fav:{},bestWeapon:'Pulse'};P=player();G={run:1,pause:0,over:0,t:0,seed,rng:new RNG(seed),rooms:new Map,rx:0,ry:0,cur:null,en:[],bs:[],eb:[],lt:[],pt:[],txt:[],rings:[],mines:[],sentries:[],decoys:[],barriers:[],links:[],keys:0,core:0,kills:0,drones:0,rift:1,combo:0,comboT:0,shake:0,flash:0,art:[],cam:{x:0,y:0}};let f=[[0,0]];G.rooms.set('0,0',room(0,0,'start'));for(let a=0;G.rooms.size<55&&a<6000;a++){let b=G.rng.p(f),dirs=[[1,0],[-1,0],[0,1],[0,-1]].sort(()=>G.rng.n()-.5);for(let[d,e]of dirs){let x=b[0]+d,y=b[1]+e;if(!G.rooms.has(rk(x,y))&&G.rng.n()<.66){let t=G.rng.n()<.12?'shop':G.rng.n()<.22?'treasure':G.rng.n()<.32?'lore':'combat';G.rooms.set(rk(x,y),room(x,y,t));f.push([x,y]);break}}}let rs=[...G.rooms.values()].filter(r=>r.x||r.y).sort((a,b)=>di(0,0,b.x,b.y)-di(0,0,a.x,a.y));rs[0].type='boss';rs[0].boss=1;rs[0].clr=0;rs.slice(1,4).forEach(r=>{r.key=1;r.type='key';r.clr=0});for(let r of G.rooms.values())for(let[d,x,y]of [['E',1,0],['W',-1,0],['S',0,1],['N',0,-1]])if(G.rooms.has(rk(r.x+x,r.y+y)))r.doors[d]=1;enter(0,0,1)}
function enter(x,y,first){let r=G.rooms.get(rk(x,y));if(!r)return;G.rx=x;G.ry=y;G.cur=r;runStats.rooms++;P.seen.add(rk(x,y));for(let[d,dx,dy]of [['E',1,0],['W',-1,0],['S',0,1],['N',0,-1]])if(r.doors[d])P.seen.add(rk(x+dx,y+dy));G.en=[];G.bs=[];G.eb=[];G.lt=[...r.loot];G.mines=[];G.sentries=[];G.decoys=[];G.barriers=[];spawnRoom(r);if(!first)toast(r.type.toUpperCase()+' room')}
function spawnRoom(r){if(r.spawn||r.clr||['start','shop','lore','treasure','shrine','rescue','forge'].includes(r.type))return;r.spawn=1;let d=diff();if(r.boss){enemy('boss',PW/2,175);toast('WARDEN CORE AWAKENED');return}let n=Math.max(2,Math.floor(4+Math.abs(r.x)+Math.abs(r.y)+G.rift*1.5+(d.enemyCount||0)));for(let i=0;i<n;i++){let x,y;do{x=R(PW-180,90);y=R(PH-180,90)}while(di(x,y,P.x,P.y)<260);enemy(pick(['drone','drone','dash','turret','split','tank','sniper','bomber','healer','shield','swarm','swarm']),x,y)}}
function enemy(type,x,y){let d={drone:[38,15,135,16,'#ff4d6d'],dash:[55,17,185,21,'#ffd166'],turret:[70,20,0,25,'#b28dff'],split:[48,18,105,24,'#6aff9e'],tank:[125,25,72,45,'#ff8fab'],sniper:[44,16,52,34,'#7df9ff'],bomber:[60,20,150,28,'#ff9f1c'],healer:[58,18,90,36,'#9cffb1'],shield:[95,24,70,42,'#9bb7ff'],swarm:[22,10,230,12,'#ff6bcb'],boss:[980,43,80,500,'#fff']}[type],q=diff(),m=(1+G.rift*.07)*q.enemyHp,elite=Math.random()<Math.min(.4,Math.max(0,.04+G.rift*.02+(q.eliteAdd||0)))&&type!='boss';if(elite)m*=1.65;G.en.push({type,x,y,vx:0,vy:0,h:d[0]*m,mh:d[0]*m,r:d[1]*(elite?1.15:1)*1.075,sp:d[2]*(elite?1.15:1)*q.enemySpeed,xp:Math.round(d[3]*(elite?2:1)),col:elite?'#ff7bff':d[4],cd:R(2),fl:0,slow:0,elite,dmg:1.02*q.enemyDamage})}
function tick(dt){if(!G.run||G.pause||G.over||$('level').style.display=='flex'||$('codex').style.display=='block'||$('shopPanel').style.display=='flex'||$('decodePanel').style.display=='flex'||$('forgeResult').style.display=='flex')return;G.t+=dt;G.rift=1+(G.t/75|0);G.comboT=Math.max(0,G.comboT-dt);if(!G.comboT)G.combo=0;G.shake=Math.max(0,G.shake-dt*18);G.flash=Math.max(0,G.flash-dt*3);G.hitFlash=Math.max(0,(G.hitFlash||0)-dt*2.8);upPlayer(dt);upDrones(dt);upDeploy(dt);upEnemies(dt);if(G.en.length>45)G.en=G.en.slice(0,45);if(G.eb.length>120)G.eb=G.eb.slice(-120);if(G.bs.length>90)G.bs=G.bs.slice(-90);if(G.pt.length>260)G.pt=G.pt.slice(-260);upBullets(dt);upLoot(dt);upFx(dt);state();ui()}
...
function hit(e,d){if(P.over>0)d*=1.45;if(!hit.last||G.t-hit.last>.035){sfx('hit');hit.last=G.t}e.h-=d;runStats.damage+=d;e.fl=.08;burst(e.x,e.y,e.col,10,3,180);text(e.x,e.y,d|0,'#fff');if(e.h<=0&&!e.dead)kill(e)}function kill(e){sfx(e.type=='boss'?'boss':'death');e.dead=1;G.kills++;G.combo++;G.comboT=3.2;P.heat=cl(P.heat+12+(e.elite?20:0),0,100);P.xp+=e.xp*(diff().xp||1)*(1+Math.min(G.combo,25)*.02);P.e=cl(P.e+P.ref,0,P.me);drop(e.x,e.y,'scrap',Math.max(1,(R(9,3)*P.scrapMul*(diff().loot||1))|0));if(Math.random()<.12)drop(e.x,e.y,'cell',1);if(e.elite&&Math.random()<.5)drop(e.x,e.y,'artifact',1);if(e.type=='split')for(let i=0;i<2;i++)enemy('swarm',e.x+R(60,-60),e.y+R(60,-60));if(P.spl)for(let i=0;i<5+P.spl;i++){let a=R(6.28);G.bs.push({x:e.x,y:e.y,vx:Math.cos(a)*520,vy:Math.sin(a)*520,r:3,l:.35,d:8*P.spl,pi:0,col:'#ffd166'})}if(e.type=='boss'){G.cur.clr=1;G.core=1;drop(e.x,e.y,'core',1);toast('Warden destroyed. Take the core.')}burst(e.x,e.y,e.col,e.elite?70:42,6,e.elite?420:290);ring(e.x,e.y,e.col,25,e.elite?190:125,.45);level()}
...
['Hull Repair',25,'Restore 55 hull.',()=>P.h<P.mh,()=>{P.h=cl(P.h+55,0,P.mh)}],
...
function state(){if(!G.cur.clr&&G.en.length==0){G.cur.clr=1;if(G.cur.key&&!G.cur.kd){G.cur.kd=1;drop(PW/2,PH/2,'key',1)}if(G.cur.type=='treasure'){drop(PW/2-30,PH/2,'scrap',60);drop(PW/2+30,PH/2,'cell',1)}toast('Room clear')}if(G.rx==0&&G.ry==0&&G.core&&di(P.x,P.y,PW/2,PH/2)<95)end(1)}
function level(){while(P.xp>=P.nx){P.xp-=P.nx;P.rank++;P.nx=P.nx*1.36+25|0;let u=[...UP].sort(()=>Math.random()-.5).slice(0,4);$('ups').innerHTML=u.map(a=>`<div class="card up"><div style="font-size:32px">${a[0]}</div><h3>${a[1]}</h3><p>${a[2]}</p></div>`).join('');[...document.querySelectorAll('.up')].forEach((el,i)=>el.onclick=()=>{u[i][3](P);$('level').style.display='none';toast(u[i][1])});$('level').style.display='flex';break}}
...
function roomTheme(){
  ...
  return t=='shop'?['#072018','#6aff9e','MARKET']:t=='lore'?['#071b2a','#62e3ff','ARCHIVE']:t=='forge'?['#241508','#ff9f1c','FORGE']:t=='shrine'?['#21190a','#ffd166','SHRINE']:t=='rescue'?['#071e22','#62e3ff','RESCUE']:t=='boss'?['#230711','#ff426b','WARDEN']:t=='key'?['#201a07','#ffd166','KEY ROOM']:t=='treasure'?['#211b08','#ffd166','TREASURE']:['#08111a','#62e3ff','DUNGEON'];
}
...
if(diff().epstein){
  let sx=EPSTEIN_IMG.width*EPSTEIN_CROP[0], sy=EPSTEIN_IMG.height*EPSTEIN_CROP[1], sw=EPSTEIN_IMG.width*EPSTEIN_CROP[2], sh=EPSTEIN_IMG.height*EPSTEIN_CROP[3];
  let halo=e.elite?'#ff7bff':'#ff4d6d';
  X.save();
  X.beginPath();
  X.arc(0,0,e.r*.98,0,Math.PI*2);
  X.closePath();
  X.clip();
  if(EPSTEIN_IMG.complete&&EPSTEIN_IMG.naturalWidth){
    X.drawImage(EPSTEIN_IMG,sx,sy,sw,sh,-e.r,-e.r,e.r*2,e.r*2);
  }else{
    let g=X.createRadialGradient(-e.r*.25,-e.r*.25,1,0,0,e.r*1.2);g.addColorStop(0,'#f2d9c0');g.addColorStop(1,'#a58a74');X.fillStyle=g;X.fillRect(-e.r,-e.r,e.r*2,e.r*2);
  }
  X.restore();
  ...
  X.stroke();
  X.strokeStyle=halo;
  X.lineWidth=e.elite?3.2:2;
  X.beginPath();
  X.arc(0,0,e.r*1.12+Math.sin(G.t*6+e.x*.02)*1.5,0,Math.PI*2);
  X.stroke();
  X.fillStyle='rgba(0,0,0,.5)';
  X.beginPath();X.arc(e.r*.18,-e.r*.05,e.r*.14,0,Math.PI*2);X.fill();
  X.beginPath();X.arc(-e.r*.15,-e.r*.05,e.r*.14,0,Math.PI*2);X.fill();
  X.restore();
...
function mini(){MX.clearRect(0,0,220,150);let ox=110,oy=75,s=8;for(let r of G.rooms.values()){if(!P.seen.has(rk(r.x,r.y)))continue;let x=ox+r.x*s,y=oy+r.y*s;MX.fillStyle=r.x==G.rx&&r.y==G.ry?'#fff':r.boss?'#ff4d6d':r.key?'#ffd166':r.type=='shop'?'#6aff9e':['shrine','rescue','forge'].includes(r.type)?'#ffd166':r.clr?'#62e3ff99':'#62e3ff33';MX.fillRect(x-4,y-4,8,8)}}

function ui(){ensureWeaponIndex();let p=(id,v)=>$(id).style.width=cl(v,0,100)+'%';$('hpT').textContent=`${P.h|0}/${P.mh}`;p('hp',100*P.h/P.mh);$('shT').textContent=`${P.s|0}/${P.ms}`;p('sh',100*P.s/P.ms);$('enT').textContent=`${P.e|0}/${P.me}`;p('en',100*P.e/P.me);$('rank').textContent=P.rank;$('xpT').textContent=`XP ${P.xp|0}/${P.nx}`;p('xp',100*P.xp/P.nx);$('rift').textContent=`${P.charName} · ${difficultyAt(selectedDifficulty).badge} · Rift ${G.rift}`;$('seed').textContent=G.seed.toString(16).toUpperCase();$('room').textContent=`${G.rx},${G.ry} ${G.cur.type}`;$('time').textContent=(G.t/60|0)+':'+String(G.t%60|0).padStart(2,'0');$('wep').textContent=weps[P.w][0];$('stats').innerHTML=`<b>${P.charName}</b> · ${P.charRole}<br><span class="muted">${P.charPassive}</span><br>Damage x${P.dm.toFixed(2)} · Armor ${P.armor}<br>${keyLabel(settings.binds.shock)} Shock ${P.sc<=0?'Ready':P.sc.toFixed(1)} · ${keyLabel(settings.binds.repair)} Repair ${P.rc<=0?'Ready':P.rc.toFixed(1)}<br>${keyLabel(settings.binds.lightning)} Chain Lightning ${P.zc<=0?'Ready':P.zc.toFixed(1)}<br>Heat ${P.heat|0}/100 · Combo x${G.combo} · Mercy ${P.mercy}<br>Kills ${G.kills} · Drones ${G.drones} · Artifacts ${G.art.length}`;$('slots').innerHTML=[['◇',P.scrap],['◆',G.keys],['✦',G.core],['🔋',P.e|0],['🤖',G.drones],['⚙️',P.rank]].map(a=>`<span class="slot">${a[0]}<b>${a[1]}</b></span>`).join('');$('quest').innerHTML=`<b>${P.charName}</b> <span class="badge">${P.charRole}</span> <span class="badge">${difficultyAt(selectedDifficulty).badge}</span><div class="muted" style="margin:6px 0 9px">${P.charSummary}</div><div class="card" style="padding:10px;margin-top:8px"><b>CURRENT OBJECTIVE</b><br>${G.core?'Escape: return to the start room and stand on the extraction gate.':G.keys>=3?'Boss unlocked: find and defeat the Warden Core.':`Find relic keys: ${G.keys}/3 collected.`}</div><div class="muted" style="margin-top:8px">Enemy clues: laser = sniper, orange ring = bomber, green beam = healer, blue aura = shield.</div><div class="muted" style="margin-top:6px">Unlocks: ${(meta.unlocks||[]).join(', ')||'none yet'}</div>`}
...
function save(){if(!P)return;if(G.cur)G.cur.loot=[...G.lt];localStorage.NEON_RELIC_FIXED=JSON.stringify({seed:G.seed,t:G.t,rx:G.rx,ry:G.ry,difficulty:selectedDifficulty,P:{...P,seen:[...P.seen]},weps:weps.map(w=>w.slice()),keys:G.keys,core:G.core,kills:G.kills,drones:G.drones,rooms:[...G.rooms],en:G.en,lt:G.lt,mines:G.mines,sentries:G.sentries,decoys:G.decoys,barriers:G.barriers,art:G.art,rift:G.rift,combo:G.combo,comboT:G.comboT});toast('Saved')}
function load(){let d=localStorage.NEON_RELIC_FIXED;if(!d)return toast('No save found');d=JSON.parse(d);setDifficulty(d.difficulty||selectedDifficulty);world(d.seed);G.t=d.t||0;G.keys=d.keys||0;G.core=d.core||0;G.kills=d.kills||0;G.drones=d.drones||0;G.rift=d.rift||1;G.combo=d.combo||0;G.comboT=d.comboT||0;G.art=d.art||[];G.rooms=new Map(d.rooms||[]);if(d.weps&&d.weps.length)weps.splice(0,weps.length,...d.weps.map(w=>w.slice()));else resetWeapons();P={...d.P,seen:new Set((d.P&&d.P.seen)||['0,0'])};P.w=Math.max(0,Math.min((P.w|0),weps.length-1));selectedCharacter=Number.isInteger(P.charId)?P.charId:selectedCharacter;stampCharacterFields(P,selectedCharacter);setCharacter(selectedCharacter);$('menu').style.display='none';$('level').style.display='none';$('codex').style.display='none';$('exitRun').style.display='block';enter(d.rx||0,d.ry||0,1);G.en=d.en||G.en;G.lt=d.lt||G.lt;G.mines=d.mines||[];G.sentries=d.sentries||[];G.decoys=d.decoys||[];G.barriers=d.barriers||[];ui();toast('Loaded '+difficultyAt(selectedDifficulty).name)}
function exitRun(){if(!P||!G.run)return;save();G.run=0;G.pause=0;$('level').style.display='none';$('codex').style.display='none';$('exitRun').style.display='none';$('menu').style.display='flex';renderCharacterSelect();renderDifficultySelect();$('start').onclick=start;$('load').onclick=load;if($('audioToggle'))$('audioToggle').onclick=toggleAudio;toast('Progress saved')}
